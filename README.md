...
The first sync reads your whole Drive once (a few minutes on
...
`drive.appdata` scopes), create a Client ID + secret, paste both
in, and connect.
...
- **Google-native files** (Docs, Sheets, Slides, Forms) don't report a byte
  ...
  Shortcuts to a regular file resolve to the target's real size, shortcuts to a native doc inherit the same caveat.
...
Working on the source directly ?
